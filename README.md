...
While a batch is active, a compact status panel stays above Pi's editor so progress remains visible when the original tool card scrolls away. It shows batch counts and role-based task names only; the original card remains the place to click a task and read its text and assistant replies. The fixed panel disappears when all batches finish.

Pi's `/reload` keeps running and queued OMP children in the same process. The new extension instance reconnects their task cards and delivers any results that finished during reload. Switching sessions or quitting Pi cancels active work; process restarts cannot resume child processes.
