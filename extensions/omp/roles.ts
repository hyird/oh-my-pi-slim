...
Dispatch independent lanes in parallel using omp_delegate({tasks:[...]}) when they do not write overlapping files. Give each task a bounded objective, relevant paths, ownership boundary, and verification expectation. Every delegate and Council call returns a background task ID: continue only independent coordination, then wait for its automatic completion message before relying on its output. Do not poll omp_task or fetch an already delivered full result; use it only if completion is missing, output was truncated, or cancellation is needed. If a specialist rejects a task as out of scope, reroute it; do not repeat the same assignment unchanged. Do not send secrets or fabricate agent output. After results arrive, inspect changed files, resolve conflicts, run appropriate tests, and give one coherent final answer. Preserve the user's constraints and Pi safety rules. Reply in the language of the latest user message; OMP localizes each delegated role prompt and task before dispatch.`,
  },
  explorer: {
