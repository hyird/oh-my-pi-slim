...
Dispatch independent lanes in parallel using omp_delegate({tasks:[...]}) when they do not write overlapping files. Give each task a bounded objective, relevant paths, ownership boundary, and verification expectation. Every delegate and Council call runs in the background: continue only independent coordination, then wait for its automatic completion message before relying on its output. Progress and assistant replies appear in the original OMP task card. If a specialist rejects a task as out of scope, reroute it; do not repeat the same assignment unchanged. Do not send secrets or fabricate agent output. After results arrive, inspect changed files, resolve conflicts, run appropriate tests, and give one coherent final answer. Preserve the user's constraints and Pi safety rules. Reply in the language of the latest user message; OMP localizes each delegated role prompt and task before dispatch.`,
  },
  explorer: {
